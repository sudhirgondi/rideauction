...
import static com.sav.ra.test.locator.BidPointToPointPageElementLocators.*;
import static com.sav.ra.test.locator.CommonBidPageElementLocators.*;

public class TestBidOnRidePointToPoint extends RideauctionSharedTestsAndUtils{

	@DataProvider(name="bidOnRidePointToPointDataProviderPositive")
	public String[][][] bidOnRidePointToPointDataProviderPositive() {
		return ExcelReader.readExcelDataArr("bidOnRidePointToPointTestData.xlsx", "pointToPointPos");
	}

	@DataProvider(name="bidOnRidePointToPointDataProviderNegative")
	public String[][][] bidOnRidePointToPointDataProviderNegative() {
		return ExcelReader.readExcelDataArr("bidOnRidePointToPointTestData.xlsx", "pointToPointNeg");
	}
	
	@Test(dataProvider="bidOnRidePointToPointDataProviderPositive")
	public void testBidOnRidePointToPointPositive(String[] dataParams) throws InterruptedException {
		int dataIndex = 0;
		...
		verifyAndClickElement(driver, POINT_TO_POINT_BID_ON_RIDE_BTN);
	...
	@Test(dataProvider="bidOnRidePointToPointDataProviderNegative")
	public void testBidOnRidePointToPointNegative(String[] dataParams) throws InterruptedException {
		int idataIndex = 0;
		...
		String numPassengers = stripDecimal(dataParams[idataIndex++]);
		String daysString = stripDecimal(dataParams[idataIndex++]);
		String hourString = stripDecimal(dataParams[idataIndex++]);
		String minutes = dataParams[idataIndex++];
		String origAddress = dataParams[idataIndex++];
		String origValidAddress = dataParams[idataIndex++];
		String destAddress = dataParams[idataIndex++];
		String destValidAddress = dataParams[idataIndex++];
		String clickForExtras = dataParams[idataIndex++];
		String extraStops = dataParams[idataIndex++];
		String numberOfStops = stripDecimal(dataParams[idataIndex++]);
